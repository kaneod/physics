...
program nexspec
  ...
  if ((iminband .lt. 1) .or. (iminband .gt. mbands)) then
  ...
  print *, "Generating a spectrum from ", w_start, "to", w_end
  
  allocate(spectrum(spectrum_points, nspins, 6))
  ...
  do ns=1,nspins
    do nk=1,nkpts
      do nb=iminband,mbands
        ...
        ! We just outright don't worry about this - let the user specify iminband.
        f_nks = 1.0d0
        !else
        !  f_nks = 0.0d0
        !end if
...
!$OMP END PARALLEL DO
  ...
  !&             //trim(adjustl(atstr(4)))
    ...
    write(300,*) '# NEXAFS core-level spectrum calculated by nexspec with CASTEP inputs.'
    write(300,*) '# Smeared with lorentzian and gaussian broadening.'
    write(300,*) '# Omega (eV) Mxx Myy Mzz Mxy Mxz Myz'
